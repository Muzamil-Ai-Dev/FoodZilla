...
export const Header = () => {
  ...
  return (
    <header
      className={cn(
        "sticky top-0 z-[100] w-full transition-all duration-200 ease-in-out",
        isScrolled 
          ? "bg-white/90 backdrop-blur-xl shadow-md py-2" 
          : "bg-white/60 backdrop-blur-lg py-3 md:py-4"
      )}
    >
        ...
        {isSearchOpen ? (
          <form onSubmit={onSearchSubmit} className="flex-1 flex items-center gap-3 md:gap-6 animate-in fade-in slide-in-from-right-5 duration-300">
            ...
            <input
              ...
              className="flex-1 text-base md:text-lg font-bold placeholder:text-gray-400 focus:outline-none bg-transparent text-brand-dark min-w-0"
              value={searchQuery}
              onChange={(e) => setSearchQuery(e.target.value)}
            />
            <button type="button" onClick={() => setIsSearchOpen(false)} className="p-2 hover:bg-gray-100 rounded-full transition-colors flex-shrink-0">
          ...
          <>
            <div className="flex items-center gap-2 md:gap-8 min-w-0">
              {/* Mobile Hamburger */}
              <button onClick={() => setIsMobileMenuOpen(true)} className="p-2 lg:hidden text-brand-dark hover:bg-gray-100 rounded-xl flex-shrink-0">
              ...
              <nav className="hidden lg:flex items-center gap-4 2xl:gap-6">
                {navLinks.map((link) => (
                  <Link 
                    key={link.name} 
                    href={link.href} 
                    className="group relative text-[13px] 2xl:text-[14px] font-bold text-gray-500 hover:text-brand-primary transition-colors whitespace-nowrap"
                  >
                    {link.name}
                    {link.name === "zilla ads" && (
            ...
            <div className="flex items-center gap-1.5 sm:gap-5 flex-shrink-0">
              ...
              <button 
                ...
                className="hidden sm:flex items-center gap-2 px-2.5 py-1.5 bg-gray-50 rounded-xl transition-all group"
              >
                ...
                <div className="text-left">
                  ...
                  <span 
                    ...
                    className="text-[12px] md:text-sm font-bold text-brand-dark truncate max-w-[80px] md:max-w-[140px] block leading-none group-hover:text-brand-primary transition-colors"
                  >
              ...
              <div className="flex items-center gap-1.5 md:gap-4 sm:border-l border-gray-100 sm:pl-4 md:pl-6">
                {/* Search Trigger */}
                <button onClick={() => setIsSearchOpen(true)} className="p-2 md:p-2.5 hover:bg-brand-secondary rounded-full text-gray-600 hover:text-brand-primary transition-colors">
                ...
                <div className="hidden lg:flex items-center gap-2 min-w-[140px] justify-end">
                  {mounted && isAuthenticated ? (
                    <Popover.Root>
                      <Popover.Trigger asChild>
                        <button className="outline-none">
                          <div className="w-9 h-9 md:w-10 md:h-10 rounded-xl md:rounded-2xl bg-brand-secondary flex items-center justify-center border-2 border-transparent hover:border-brand-primary transition-all overflow-hidden shadow-sm">
                            {user?.avatarUrl ? <Image src={user.avatarUrl} alt={user.name || "User"} width={40} height={40} className="w-full h-full object-cover" /> : <User className="w-5 h-5 md:w-6 md:h-6 text-brand-dark" />}
                          </div>
                        </button>
                      </Popover.Trigger>
                      <Popover.Portal>
                    ...
                    <>
                      <Link href="/login"><Button variant="ghost" className="font-bold text-[10px] md:text-[11px] h-9 md:h-10 px-3 md:px-4">Log in</Button></Link>
      ...
      {/* Mobile Menu */}
      {isMobileMenuOpen && (
        <div className="fixed inset-0 z-[200] w-screen h-dvh bg-white p-6 space-y-8 animate-in slide-in-from-left duration-300 overflow-y-auto">
          <div className="flex items-center justify-between">
             <span className="text-xl font-black text-brand-primary uppercase italic">foodzilla</span>
             <button onClick={() => setIsMobileMenuOpen(false)} className="p-2 bg-gray-100 rounded-full"><X className="w-6 h-6 text-gray-500" /></button>
          </div>
          <nav className="grid gap-4">
            {navLinks.map((link) => (
              <Link key={link.name} href={link.href} onClick={() => setIsMobileMenuOpen(false)} className="text-2xl font-black text-brand-dark italic uppercase">{link.name}</Link>
            ))}
          </nav>
          <div className="pt-8 border-t border-gray-100 flex flex-col gap-4">
             {!isAuthenticated ? (
               <>
                  <Link href="/login" onClick={() => setIsMobileMenuOpen(false)}><Button variant="outline" className="w-full py-6 rounded-2xl font-black uppercase tracking-widest">Log In</Button></Link>
               ...
               <button onClick={() => { logout(); setIsMobileMenuOpen(false); router.push("/"); }} className="text-xl font-black text-red-600 italic uppercase">Logout</button>
             )}
          </div>
        </div>
      )}
